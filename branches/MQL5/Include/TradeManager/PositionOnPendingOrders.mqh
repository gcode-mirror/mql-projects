...
public:
   ...
   void SetPosPriceClose(double ask,double bid);   //сохраняет цену закрытия позиции
   
   void SetPosDTPriceClose(datetime time); //сохраняет время закрытия позиции
   
   void SetPosProfit();       //сохраняет профит позиции
...
void CPosition::SetPosPriceClose(double ask,double bid) //сохраняет цену закрытия позиции
 {
  switch (_type)
   {
    case OP_SELL:
     _posClosePrice = ask;//= SymbolInfoDouble(_symbol, SYMBOL_BID);   //сохраняем цену закрытия позиции
    break;
    case OP_BUY:
     _posClosePrice = bid;//SymbolInfoDouble(_symbol, SYMBOL_ASK);   //сохраняем цену закрытия позиции    
    break;
   }
 }
 
void CPosition::SetPosDTPriceClose(datetime time)  //сохраняет время закрытия позиции
 {
    _posCloseTime = time;
 }

void CPosition::SetPosProfit(void)  //сохраняет прибыль позиции
 {
  switch (_type)
   {
    case OP_SELL:
