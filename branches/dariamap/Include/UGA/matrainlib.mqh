...
CrossEMA        trade;               //объект класса CrossEMA

double          traindd;             // Максимально возможная просадка баланса в тренировке

ENUM_TM_POSITION_TYPE   my_signal;   //переменная для хранения торгового сигнала
...
//--- ГА выбирает пару
  ...
  // MAlong =iMA(s,tf,(int)MathRound(Colony[2][chromos]*MaxMAPeriod)+1,0,MODE_SMA,PRICE_OPEN);
   ...
   trade.UpdateHandle(SLOW_EMA,(int)MathRound(Colony[1][chromos]*MaxMAPeriod)+1); //редактируем период медленного EMA
   trade.UpdateHandle(FAST_EMA,(int)MathRound(Colony[1][chromos]*MaxMAPeriod)+1); //редактируем период быстрого EMA   
...
//--- Для нейросети, использующей исторические данные - откуда начинаем их копировать
   for(from=b;from>=1;from--)
     {
      my_signal = trade.GetSignal(true,from);  //получаем торговый сигнал
...
//--- промежуточное звено между колонией генов и оптимизируемыми параметрами
  ...
  if(!trade.UploadBuffers(from)) //получаем торговый сигнал
    return;
