...
int OnInit()
  {
   ...
   // создаем события
   event.AddNewEvent(_Symbol,_Period,event.GenUniqEventName("новый экстремум",_Symbol,_Period) );
       ...
       if ( extr.isExtremum(extrHigh,extrLow,time[i],false) )
        {
          ...
          if (extrHigh.time >= time[i])
           {
            ...
            bufferFormedExtrHigh[i] = extrHigh.price; // удалить к хуям
          ...
          if (extrLow.time >= time[i])
           {        
            ...
            bufferFormedExtrLow[i] = extrLow.price;  // удалить к хуям
