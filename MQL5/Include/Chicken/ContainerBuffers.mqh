...
//|        Копирвание значений производится на каждом новом баре для каждого таймфрейма                                                         |
//+----------------------------------------------------------------------------------------+
class CContainerBuffers
{
 ...
 public:
               ...
               bool Update();
               bool isAvailable(ENUM_TIMEFRAMES period); //отладка
 ...
 { 
  if(_handleAvailable[i])
  {
   ...
   { 
    ...
    if(CopyLow(_Symbol, bufferLow.GetTF(), 1, DEPTH_MAX, bufferLow.buffer) < DEPTH_MAX )     // буфер максимальных цен всех сформированных баров на заданую глубину
    {
    ...
    if(CopyClose(_Symbol, bufferClose.GetTF(), 1, DEPTH_MIN, bufferClose.buffer)   < DEPTH_MIN)     // буфер минимальных цен всех сформированных баров на заданую глубину
    {
     ...
     PrintFormat("%s Ошибка при копировании буфера Low на периоде %s", MakeFunctionPrefix(__FUNCTION__), PeriodToString(bufferClose.GetTF()));
     return false;
    }   
    if(CopyBuffer(_handlePBI[i], 4, 0, DEPTH_MIN, bufferPBI.buffer)      < DEPTH_MIN)          // последнее полученное движение
    {
     ...
     log_file.Write(LOG_DEBUG, StringFormat("%s Ошибка при копировании буфера Low на периоде %s", MakeFunctionPrefix(__FUNCTION__), PeriodToString(bufferPBI.GetTF())));
     PrintFormat("%s Ошибка при копировании буфера Low на периоде %s", MakeFunctionPrefix(__FUNCTION__), PeriodToString(bufferPBI.GetTF()));
     return false;
    }
   }
   else
