...
bool CColoredTrend::CountMoveType(int bar, int start_pos, SExtremum& extremum, ENUM_MOVE_TYPE topTF_Movement = MOVE_TYPE_UNKNOWN)
{
 ...
 {
  if(PRINT) PrintFormat("%s bar = %d, закончился тренд вверх(началась коррекция вниз), текущее закрытие=%.05f; текущее открытия=%.05f", TimeToString(time_buffer[0]), bar, buffer_Rates[2].close, buffer_Rates[2].open);
  //PrintFormat("%s num0 = {%d, %.05f}; num1 = {%d, %.05f}; num2 = {%d, %.05f};", TimeToString(time_buffer[0]), num0.direction, num0.price, num1.direction, num1.price, num2.direction, num2.price);
 ...
 {
  if(PRINT) PrintFormat("%s bar = %d, закончился тренд вниз(началась коррекция вверх), текущее закрытие=%.05f; текущее открытия=%.05f", TimeToString(time_buffer[0]), bar, buffer_Rates[2].close, buffer_Rates[2].open);
  //PrintFormat("%s num0 = {%d, %.05f}; num1 = {%d, %.05f}; num2 = {%d, %.05f};", TimeToString(time_buffer[0]), num0.direction, num0.price, num1.direction, num1.price, num2.direction, num2.price);
      ...
      isCorrectionEnds(buffer_Rates[AMOUNT_OF_PRICE-1].close, enumMoveType[bar], start_pos))                       
 {
  ...
  if(PRINT) PrintFormat("%s bar = %d, закончилася коррекция вверх(начался тренд вниз), последняя цена=%.05f меньше последнего экстремума=%.05f", TimeToString(time_buffer[0]), bar, buffer_Rates[2].close, lastOnTrend.price);
  //PrintFormat("%s num0 = {%d, %.05f}; num1 = {%d, %.05f}; num2 = {%d, %.05f};", TimeToString(time_buffer[0]), num0.direction, num0.price, num1.direction, num1.price, num2.direction, num2.price);
  //PrintFormat("%s", MoveTypeToString(enumMoveType[bar]));
  previous_move_type = enumMoveType[bar];
  return (true);
      ...
      isCorrectionEnds(buffer_Rates[AMOUNT_OF_PRICE-1].close, enumMoveType[bar], start_pos))
 {
  enumMoveType[bar] = (topTF_Movement == MOVE_TYPE_FLAT) ? MOVE_TYPE_TREND_UP_FORBIDEN : MOVE_TYPE_TREND_UP;
  //PrintFormat("%s", MoveTypeToString(enumMoveType[bar]));
  if(PRINT) PrintFormat("%s bar = %d, закончилася коррекция вниз(начался тренд вверх), последняя цена=%.05f больше последнего экстремума=%.05f", TimeToString(time_buffer[0]), bar, buffer_Rates[2].close, lastOnTrend.price);
  //PrintFormat("%s num0 = {%d, %.05f}; num1 = {%d, %.05f}; num2 = {%d, %.05f};", TimeToString(time_buffer[0]), num0.direction, num0.price, num1.direction, num1.price, num2.direction, num2.price);
  //PrintFormat("%s", MoveTypeToString(enumMoveType[bar]));
  previous_move_type = enumMoveType[bar];
  return (true);
