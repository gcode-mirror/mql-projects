...
class CisNewBar 
  {
   ...
   public:
      ...
      void              CisNewBar(ENUM_TIMEFRAMES period);      // Конструктор CisNewBar с параметрами
      void              CisNewBar(string symbol, ENUM_TIMEFRAMES period);      // Конструктор CisNewBar с параметрами
...
//| REMARK: no.                                                      |
//+------------------------------------------------------------------+
void CisNewBar::CisNewBar(string symbol)
  {
   m_retcode=0;         // Код результата определения нового бара 
   m_lastbar_time=0;    // Время открытия последнего бара
   m_new_bars=0;        // Количество новых баров
...
//| REMARK: no.                                                      |
//+------------------------------------------------------------------+
void CisNewBar::CisNewBar(ENUM_TIMEFRAMES period)
  {
   m_retcode=0;         // Код результата определения нового бара 
   m_lastbar_time=0;    // Время открытия последнего бара
   m_new_bars=0;        // Количество новых баров
...
//+------------------------------------------------------------------+
//| Конструктор CisNewBar с параметрами                              |
//| INPUT:  no.                                                      |
//| OUTPUT: no.                                                      |
//| REMARK: no.                                                      |
//+------------------------------------------------------------------+
void CisNewBar::CisNewBar(string symbol, ENUM_TIMEFRAMES period)
  {
   m_retcode=0;         // Код результата определения нового бара 
   m_lastbar_time=0;    // Время открытия последнего бара
   m_new_bars=0;        // Количество новых баров
...
bool CisNewBar::isNewBar(datetime newbar_time)
  {
   ...
   //---
   MqlRates buffer_rates[1];
   CopyRates(m_symbol, m_period, newbar_time, 1, buffer_rates);
   newbar_time = buffer_rates[0].time;
...
int CisNewBar::isNewBar()
  {
   //---Используем первый тип запроса на появление нового бара для завершения анализа:
   if(!isNewBar(TimeCurrent())) return(0);
