...
#define DEFAULT_PERCENTAGE_ATR 1.0   // по умолчанию новый экстремум по€вл€етс€ когда разница больше среднего бара
...
//-----------------------------------------------------------------
//функци€ возвращает количество новых экстремумов на данном баре
//-----------------------------------------------------------------
int CExtremum::isExtremum(SExtremum& extr_array [], datetime start_pos_time = __DATETIME__, bool now = true)
{
 ...
 if (now) // за врем€ жизни бара цена close проходит все его значени€ от low до high
 {        // соответсвено если на данном баре есть верхний экстремум то он будет достигнут когда close будет max  и наобот с low
  high = buffer[0].close;
  low = buffer[0].close;
 }
 else    // во врем€ работы на истории мы смотрим на бар один раз соотвественно нам сразу нужно узнать его максимум и минимум
 {
  high = buffer[0].high;
  low = buffer[0].low;
 }
 
 if ((extremums[0].direction == 0 ) // ≈сли экстремумов еще нет то говорим что сейчас экстремум
...
int CExtremum::RecountExtremum(datetime start_pos_time = __DATETIME__, bool now = true)
{
 ...
 if(count_new_extrs > 0)   // если по€вились новые экстремумы
 {
  for(int i = 0; i < 2; i++) // идем по массиву экстремумов
  {
   ...
   {
    if (new_extr[i].direction == extremums[0].direction) // если новый экстремум в том же напрвлении, что и последний, то обновл€ем
    {
     extremums[0] = new_extr[i];
    }
    else                                                 // если новый экстремум в противоположном напрвлении, от последнего, сдвигаем все и добавл€ем новый
    {
...
double CExtremum::AverageBar (datetime start_pos)  // подгружаем значени€ с индикатора
{
