...
int OnInit()
  {
   ...
   fileHandle = FileOpen("FLAT_STAT_SECOND/FLAT_D_" + _Symbol+"_" + PeriodToString(_Period) + ".txt", FILE_WRITE|FILE_COMMON|FILE_ANSI|FILE_TXT, "");
      ...
      if (mode == 0)
       {
        extr = 1;
        // если сейчас флэт 
        if ( IsFlatD(extr_container.GetFormedExtrByIndex(0,EXTR_HIGH),extr_container.GetFormedExtrByIndex(1,EXTR_HIGH),
...
int IsFlatD (CExtremum *high0,CExtremum *high1,CExtremum *low0, CExtremum *low1) // флэт D
 {
...
void SaveStatToFile ()
 { 
  FileWriteString(fileHandle,"Статистика по флэту типа D: " + _Symbol + "\n");
