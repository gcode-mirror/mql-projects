...
// массив ценовых уровней, которых нужно пробить
...
int OnInit()
  {
   ...
   fileHandle = FileOpen("FLAT_STAT_SECOND/FLAT_E_" + _Symbol+"_" + PeriodToString(_Period) + ".txt", FILE_WRITE|FILE_COMMON|FILE_ANSI|FILE_TXT, "");
      ...
      if (mode == 0)
       {
        extr = 1;
        // если сейчас флэт 
        if ( IsFlatE(extr_container.GetFormedExtrByIndex(0,EXTR_HIGH),extr_container.GetFormedExtrByIndex(1,EXTR_HIGH),
...
int IsFlatE (CExtremum *high0,CExtremum *high1,CExtremum *low0, CExtremum *low1) // флэт E
 {
...
void SaveStatToFile ()
 { 
  FileWriteString(fileHandle,"Статистика по флэту типа E: " + _Symbol + "\n");
