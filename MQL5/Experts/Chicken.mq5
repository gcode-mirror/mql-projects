...
void OnTick()
{
 ...
 if(buffer_pbi[0] == MOVE_TYPE_FLAT && index_max != -1 && index_min != -1)
 {
  ...
  {
   //PrintFormat(" ≈сть позици€ стоп = %.05f, бид = %.05f", ctm.GetPositionStopLoss(_Symbol), curBid);
   /*
   if (ctm.GetPositionStopLoss(_Symbol) > curBid)
   {
    PrintFormat(" ≈сть позици€. —топ = %.05f, бид = %.05f", ctm.GetPositionStopLoss(_Symbol), curBid);
   } 
   */ 
   if(ctm.GetPositionType(_Symbol) == OP_SELLSTOP && ctm.GetPositionStopLoss(_Symbol) < curAsk) 
   {
    log_file.Write(LOG_DEBUG, StringFormat(" ≈сть позици€. —топ = %.05f, аск = %.05f", ctm.GetPositionStopLoss(_Symbol), curAsk));
    slPrice = curAsk;
    ctm.ModifyPosition(_Symbol, slPrice, 0); 
   }
   if(ctm.GetPositionType(_Symbol) == OP_BUYSTOP  && ctm.GetPositionStopLoss(_Symbol) > curBid) 
   {
    log_file.Write(LOG_DEBUG, StringFormat(" ≈сть позици€. —топ = %.05f, бид = %.05f", ctm.GetPositionStopLoss(_Symbol), curBid));
    slPrice = curBid;
    ctm.ModifyPosition(_Symbol, slPrice, 0); 
   }
  }
