...
int divHandle;             // указатель на индикатор.
int stoHandle;

int firstBar;              // индекс бара, с которго начинаются вычисления.

double mainDiv[];          // массив расхождения/схождения стохастика.
double priceDiv[];         // массив цен расхождения/схождения.
double price;              // цена открываемой позиции.
double point = Point();
ENUM_ORDER_TYPE orderType; // тип открываемой позиции.

CisNewBar nb;              // экземпляр класса CisNewBar
divSignals ds;             // экземпляр класса divSignals
...
int OnInit()
  {
   ...
   ArraySetAsSeries(mainDiv, true); // переопределение направления массива.
...
void OnDeinit(const int reason)
  {
   ...
   Print("Хэндл (указатель) divHandle очищен"); 
...
void OnTick()
  {   
   ...
   if (CopyClose(_Symbol, 0, 0, deep, priceDiv) < 0) // заполнение и проверка массива с ценами.
   {
    Print("Ошибка заполнения массива priceDiv");
    return;
   }
   if (CopyBuffer(stoHandle, MAIN_LINE, 0, deep, mainDiv) < 0) // заполнение и проверка массива основной линии.
   {
    Print("Ошибка заполнения массива mainDiv");
    return;
   }
   ...
   if (ds.Divergence(priceDiv, mainDiv, firstBar, deep) == true) // условия для начала продажи (SELL).
   {
    orderType = ORDER_TYPE_SELL;
   }
   if (ds.Convergence(priceDiv, mainDiv, firstBar, deep) == true) // условия для начала покупки (BUY).
   {
