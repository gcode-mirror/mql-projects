      ...
      if ( GreatOrEqualDoubles (SymbolInfoDouble(_Symbol,SYMBOL_BID),top_point) )
       {
        ...
        //Print("Режим 3. А я нашел пробитие и поставил 0");
     ...
     if (flatType != 0)
      {
       ...
       //Print("Режим 2. А я все посчитал и поставил 3");
       calcMode = 3;
   ...
   if (sparam == formedExtrHighEvent || sparam == formedExtrLowEvent)
   {
    ...
    if (calcMode == 0)
     { 
      ...
      if (trendNow)
      {
       //Print("Режим 0. Да, это тренд! Поставил 1");
       trend.GetTrendByIndex(0).ShowTrend();
    ...
    else if (calcMode == 1||calcMode == 3)
    {
     ...
     if (!trendNow)
     {
       //Print("Режим 1. А я нашел флет и поставил 2 ");
