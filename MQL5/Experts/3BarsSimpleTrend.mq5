...
bool             tendM5;                           // тенденция на M5
bool             tendM15;                          // тенденция на M15
bool             tendH1;                           // тенденция на H1
...
{     
 ...
 {   
  ...
  if (  ((beatM5=IsExtremumBeaten(1,BUY)) && (lastTrendPBI_1==BUY||usePBI==PBI_NO)) || 
        ...
        (( (tendM5=GetLastCloseDirection(PERIOD_M5,1)) == BUY)  && (lastTrendPBI_1==BUY||usePBI==PBI_NO)) ||
        (( (tendM15=GetLastCloseDirection(PERIOD_M15,2)) == BUY) && (lastTrendPBI_2==BUY||usePBI==PBI_NO)) || 
        (( (tendH1=GetLastCloseDirection(PERIOD_H1,3)) == BUY)  && (lastTrendPBI_3==BUY||usePBI==PBI_NO))                
        )
   {
 ...
 {                     
  ...
  if ( ((beatM5=IsExtremumBeaten(1,SELL)) && (lastTrendPBI_1==SELL||usePBI==PBI_NO)) || 
       ...
       (( (tendM5=GetLastCloseDirection(PERIOD_M5,1)) == SELL)  && (lastTrendPBI_1==SELL||usePBI==PBI_NO)) ||
       (( (tendM15=GetLastCloseDirection(PERIOD_M15,2)) == SELL) && (lastTrendPBI_2==SELL||usePBI==PBI_NO)) ||
       (( (tendH1=GetLastCloseDirection(PERIOD_H1,3)) == SELL)  && (lastTrendPBI_3==SELL||usePBI==PBI_NO))        
       )  
  {                
...
bool Upload19LinesBuffers ()   // получает последние значения уровней
 ...
 double GetClosestLevel (int direction) 
  ...
  int  GetMACDSignal (int handleMACD)
   ...
   int GetLastCloseDirection (ENUM_TIMEFRAMES tf, int indexExtr)
    {
     ENUM_EXTR_USE lastExtr;
     double closePrices[];
     double openPrices[];
     // если не удалось скопировать все 3 бара цен открытия и закрытия
     if (CopyClose(_Symbol,tf,1,3,closePrices) < 3 || CopyOpen(_Symbol,tf,1,3,openPrices) < 3)
      {
       Print ("Не удалось скопировать цены закрытия трех последних баров");
       return (0);
      }
     // определяем тип последнего экстремума
     lastExtr = blowInfo[indexExtr].GetLastExtrType();
     switch (lastExtr)
      {
       case EXTR_LOW:
        if ( (LessDoubles( closePrices[2],closePrices[1]) && LessDoubles( closePrices[2],closePrices[0])) &&   // если цена закрытия на последнем баре ниже цен закрытия на двух предыдущих
             (LessDoubles( closePrices[2],openPrices[2])  && LessDoubles( closePrices[1],openPrices[1]) && LessDoubles( closePrices[0],openPrices[0])) // если на 3-х барах тенденция вниз
           ) 
         {
          return (-1);  // то возвращает сигнал на SELL
         }
       break;
       case EXTR_HIGH:
        // если цена закрытия последнего сформированного бара выше цен закрытия двух предыдущих
        if ( (GreatDoubles ( closePrices[2], closePrices[1]) && GreatDoubles( closePrices[2],closePrices[0])) && // если цена закрытия на последнем баре выше цен закрытия на двух предыдущих
             (GreatDoubles( closePrices[2],openPrices[2]) && GreatDoubles( closePrices[1],openPrices[1]) && GreatDoubles( closePrices[0],openPrices[0])) // если на 3-х барах тенденция вверх
           )
         {
          return (1);  // то возвращаем сигнал на BUY
         }
       break;
      }
     return (0);     
