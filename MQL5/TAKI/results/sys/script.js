//+------------------------------------------------------------------+
//| —крипт переводит Unix time в формат √ригорианского календар€     |
//+------------------------------------------------------------------+
 ...
 (
  ...
  1127,1155,1186,1216,1247,1277,1308,1339,1369,1400,1430,1461 
   ...
   var _4years_days = 1461;  // количество дней в первых 4-х годах (дальше ситуаци€ по циклу повтор€етс€)
 ...
 // переменные дл€ вычислени€ текущего положени€ курсора (баланса, времени)

   var _max_balance,_min_balance;
   var _start_time, _finish_time;
   var _balance_range,_time_range;

 //дл€ делени€ числа без остатка
 function div(val, by)
  {
 ...
 function   ConvertUnixToTime (unix_time)
  {
	...
	// получаем мес€ц, день и модифицируем год
	...
	if (tmp > 0)
         {
           _day     =  _day % _4years_days - N_DAYS[tmp-1];
           if (_day == 0)
             _day = 1;
         }
	else
         {
          _day     =  _day % _4years_days;
          if (_day == 0)
             _day = 1;
         }
  }

 // функци€ переводит unix врем€ в формат григорианского календар€ в виде строки
function   TimeToString (unix_time)
 ...
 // функци€ получает значение баланса и времени в текущем положении курсора
function GetPoints() 
 {
  ...
  if (y >= 20 && y <= 306 && x >= 0 && x <= 688) 
    { 
     ...
     graph.alt = 'Ѕаланс: '+ value_balance.toFixed(2)+' '+_valuta+'\n¬рем€: '+ TimeToString(value_time); 
    } 
  }


//---- функци€ при загрузке скрипта 
function     OnLoad (max_balance,min_balance,start_time,finish_time,valuta)
 {
...
//---- функци€ измен€ет состо€ние иконки кнопки Print

function ChangePrintButton (mode)
 {
   if (mode)
     this.printButton.src = "sys\\print_over.jpg";
   else
     this.printButton.src = "sys\\print_out.jpg";
 }