...
int OnInit()
  {
   ...
   fileHandle = FileOpen("FLAT_STAT_SECOND/FLAT_B_" + _Symbol+"_" + PeriodToString(_Period) + ".txt", FILE_WRITE|FILE_COMMON|FILE_ANSI|FILE_TXT, "");
...
void OnChartEvent(const int id,         // идентификатор события  
      ...
      if (mode == 0)
       {
        extr = 1;
        // если сейчас флэт 
        if ( IsFlatB(extr_container.GetFormedExtrByIndex(0,EXTR_HIGH),extr_container.GetFormedExtrByIndex(1,EXTR_HIGH),
 ...
 int IsFlatB (CExtremum *high0,CExtremum *high1,CExtremum *low0, CExtremum *low1) // флэт B
 {
...
void SaveStatToFile ()
 { 
  FileWriteString(fileHandle,"Статистика по флэту типа B: " + _Symbol + "\n");
