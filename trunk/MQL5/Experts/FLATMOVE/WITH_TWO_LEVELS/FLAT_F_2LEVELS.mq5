...
int OnInit()
  {
   ...
   fileHandle = FileOpen("FLAT_STAT_SECOND/FLAT_F_" + _Symbol+"_" + PeriodToString(_Period) + ".txt", FILE_WRITE|FILE_COMMON|FILE_ANSI|FILE_TXT, "");
      ...
      if (mode == 0)
       {
        extr = 1;
        // если сейчас флэт 
        if ( IsFlatF(extr_container.GetFormedExtrByIndex(0,EXTR_HIGH),extr_container.GetFormedExtrByIndex(1,EXTR_HIGH),
...
int IsFlatF (CExtremum *high0,CExtremum *high1,CExtremum *low0, CExtremum *low1) // флэт F
 {
...
void SaveStatToFile ()
 { 
  FileWriteString(fileHandle,"Статистика по флэту типа F: " + _Symbol + "\n");
