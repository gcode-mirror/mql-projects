...
;//процент, насколько бар M15 больше среднего значения
...
bool firstUploadedM5 = false; // флаг первой загрузки трендов
bool firstUploadedM15 = false; // флаг первой загрузки трендов
...
void OnTick()
{
 ...
 // если еще не загружены экстремумы M1
 if (!firstUploadedM1)
  firstUploadedM1 = trendM1.UploadOnHistory();
 // если еще не загружены экстремумы M5
 if (!firstUploadedM5)
  firstUploadedM5 = trendM5.UploadOnHistory();
 // если еще не загружены экстремумы M15
 if (!firstUploadedM15)
  firstUploadedM15 = trendM15.UploadOnHistory();    
