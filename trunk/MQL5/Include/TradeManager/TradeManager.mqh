...
void CTradeManager::OnTick()
{
 ...
 for(int i = total - 1; i >= 0; i--) // по массиву НАШИХ позиций
 {
  ...
  {
<<<<<<< .mine   log_file.Write(LOG_DEBUG, StringFormat("%s Нет ордера-StopLoss", MakeFunctionPrefix(__FUNCTION__)));
=======   log_file.Write(LOG_DEBUG, StringFormat("%s Нет ордера-StopLoss, удаляем позицию [%d]", MakeFunctionPrefix(__FUNCTION__), i));
>>>>>>> .theirs   log_file.Write(LOG_DEBUG, StringFormat("%s, удаляем позицию [%d]", MakeFunctionPrefix(__FUNCTION__), i));
