...
double CExtremum::AveregeBar (ENUM_TIMEFRAMES tf, int period, datetime start_pos)
{
 ...
 {
  PrintFormat("%s Ќе удалось загрузить цены дл€ самосто€тельного подсчета ATR. TF = %s. Error = %s", __FUNCTION__, EnumToString((ENUM_TIMEFRAMES)_tf_period), GetLastError());
 }
