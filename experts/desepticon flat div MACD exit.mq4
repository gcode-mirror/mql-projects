...
int start(){
	for (frameIndex = startTF; frameIndex < finishTF; frameIndex++){
     ...
     {
      //----------------------------------------------------
      // если есть открытые позиции, возможно их надо закрыть
      //----------------------------------------------------
      if (ExistPositions("", -1, _MagicNumber))
      {
       if (isMACDPit(Elder_Timeframe, eldFastMACDPeriod, eldSlowMACDPeriod) > 0) ClosePositions("", OP_BUY , _MagicNumber); // росло, начало падать
       if (isMACDPit(Elder_Timeframe, eldFastMACDPeriod, eldSlowMACDPeriod) < 0) ClosePositions("", OP_SELL, _MagicNumber); // падало, начало расти
      }
      //----------------------------------------------------
