...
//|                                             http://www.saita.net |
//+------------------------------------------------------------------+
int DesepticonBreakthrough(int iDirection, int timeframe)
{
    total=OrdersTotal();
    if (iDirection < 0 && trendDirection[frameIndex] < 0)
    {
     ...
     {
     //if (Bid < iMA(NULL, Elder_Timeframe, eld_EMA2, 0, 1, 0, 0) || flag)
    ...
    if (iDirection > 0 && trendDirection[frameIndex] > 0)
    {
     ...
     {
     //if (Ask > iMA(NULL, Elder_Timeframe, eld_EMA2, 0, 1, 0, 0) || flag)
      if (total < 1) // Нету открытых ордеров -> ищем возможность открытия
      { 
